...
export default async function Home() {
  ...
  return (
    <div className="flex flex-col min-h-screen min-w-screen max-w-screen">
      ...
      <p className="abosolute bottom-0 p-1 bg-yellow-300 text-white rounded-full w-6 flex justify-center itens-center">3</p>
    ...
    justify-center items-center gap-4">
        {data.map((item, idx) => {
          return <div key={idx} className="flex-1 items-center flex flex-col min-w-56 max-w-56 p-5 border-2 rounded-2xl border-yellow-500 min-h-80 max-h-80 justify-between gap-2">
            ...
            <button className="min-w-full bg-white border-2 rounded-md px-4 py-2 text-yellow-500">Adicionar ao Carrinho</button>
          </div>
        })}
      </div>
    </div>
